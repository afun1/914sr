...
export default function Hierarchy({ userRole }: HierarchyProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {showAssignmentModal && selectedSupervisor && (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-md w-full max-h-[80vh] overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-600">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                {assignmentAction === 'assign' ? 'Assign Users' : 'Unassign User'}
              </h3>
              <button
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                {assignmentAction === 'assign' 
                  ? `Assign ${selectedSupervisor.role === 'supervisor' ? 'managers or users' : 'users'} to ${getDisplayName(selectedSupervisor)}`
                  : `Remove assignment from ${getDisplayName(selectedSupervisor)}`
                }
              </p>
              ...
              {assignmentAction === 'assign' ? (
                <div className="space-y-4">
                  {/* Available to Assign */}
                  <div>
                    <h4 className="font-medium text-gray-900 dark:text-white mb-2">Available to Assign:</h4>
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {getUnassignedUsers(selectedSupervisor.role || 'user').map(user => (
                        <div key={user.id} className="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-600 rounded-lg">
                          <div>
                            <div className="font-medium text-gray-900 dark:text-white">
                              {getRoleIcon(user.role)} {getDisplayName(user)}
                            </div>
                            <div className="text-sm text-gray-500">{user.email}</div>
                            <div className="text-xs text-gray-400">{user.role}</div>
                          </div>
                          <button
                            onClick={() => handleAssignment(selectedSupervisor.id, user.id, 'assign')}
                      ...
                      {getUnassignedUsers(selectedSupervisor.role || 'user').length === 0 && (
                        <div className="text-center py-4 text-gray-500 dark:text-gray-400 text-sm">
                          No unassigned {selectedSupervisor.role === 'supervisor' ? 'managers or users' : 'users'} available
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Currently Assigned */}
                  <div>
                    <h4 className="font-medium text-gray-900 dark:text-white mb-2">Currently Assigned:</h4>
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {getAllAssignedUsers(selectedSupervisor.id).map(user => (
                        <div key={user.id} className="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-600 rounded-lg bg-green-50 dark:bg-green-900/20">
                          <div>
                            <div className="font-medium text-gray-900 dark:text-white">
                              {getRoleIcon(user.role)} {getDisplayName(user)}
                            </div>
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() => {
                                ...
                                setReassignFrom(selectedSupervisor)
                            ...
                            <button
                              onClick={() => handleAssignment(selectedSupervisor.id, user.id, 'unassign')}
                      ...
                      {getAllAssignedUsers(selectedSupervisor.id).length === 0 && (
                        <div className="text-center py-4 text-gray-500 dark:text-gray-400 text-sm">
                  ...
                  {/* Assigned to Others (Available for Reassignment) */}
                  <div>
                    <h4 className="font-medium text-gray-900 dark:text-white mb-2">Assigned to Others:</h4>
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                      {getAssignedToOthers(selectedSupervisor.role || 'user').map(user => {
                        const currentAssignor = getCurrentAssignor(user.id)
                        return (
                          <div key={user.id} className="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-600 rounded-lg bg-yellow-50 dark:bg-yellow-900/20">
                            <div>
                              <div className="font-medium text-gray-900 dark:text-white">
                                {getRoleIcon(user.role)} {getDisplayName(user)}
                              </div>
                              ...
                              <div className="text-xs text-gray-400">
                                Currently assigned to: {currentAssignor ? getDisplayName(currentAssignor) : 'Unknown'}
                              </div>
                            </div>
                            <button
                              onClick={() => {
                                setReassignTarget(user)
                                setReassignFrom(currentAssignor)
                                setShowReassignConfirm(true)
                              }}
                      ...
                      {getAssignedToOthers(selectedSupervisor.role || 'user').length === 0 && (
                        <div className="text-center py-4 text-gray-500 dark:text-gray-400 text-sm">
                          No users assigned to others available for reassignment
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="p-3 border border-gray-200 dark:border-gray-600 rounded-lg">
                    <div className="font-medium text-gray-900 dark:text-white">
                      {getRoleIcon(selectedAssignee.role)} {getDisplayName(selectedAssignee)}
                    </div>
                    <div className="text-sm text-gray-500">{selectedAssignee.email}</div>
                    <div className="text-xs text-gray-400">{selectedAssignee.role}</div>
                  </div>
                  <div className="flex space-x-3">
                    <button
                      onClick={() => handleAssignment(selectedSupervisor.id, selectedAssignee.id, 'unassign')}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-md w-full">
            ...
            <div className="p-6">
              <p className="text-gray-600 dark:text-gray-400 mb-6">
                ...
                <strong className="text-gray-900 dark:text-white">{getDisplayName(reassignTarget)}</strong>
                {' '}from{' '}
                <strong className="text-gray-900 dark:text-white">{getDisplayName(reassignFrom)}</strong>
                {' '}to{' '}
                <strong className="text-gray-900 dark:text-white">{getDisplayName(selectedSupervisor)}</strong>?
              </p>
              ...
              <div className="flex space-x-3">
                ...
                <button
                  onClick={() => handleReassignment(selectedSupervisor.id, reassignTarget.id, reassignFrom.id)}

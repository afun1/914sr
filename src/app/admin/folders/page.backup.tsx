...
export default function AdminFoldersPage() {
  ...
  const fetchFolders = async () => {
    try {
      ...
      // Process folders for display - Show Sparky Screen Recordings folders AND their nested folders
      const processedFolders = allFolders
        .filter((folder: any) => {
          const folderName = (folder.name || '').toLowerCase()
          // Include folders with "Sparky" and "Screen" and "Recording" in the name
          const isSparkyFolder = folderName.includes('sparky') && 
                                folderName.includes('screen') && 
                                folderName.includes('recording')
          
          // Also include specific nested folders we want to show (like John Bradshaw)
          const isNestedFolder = folderName.includes('john bradshaw') ||
                                folderName.includes('bradshaw')
          
          return isSparkyFolder || isNestedFolder
        })
        .map((folder: any) => ({
          ...folder,
        ...
        .sort((a: any, b: any) => {
          // Main repo folders always come first
          if (a.isMainFolder && !b.isMainFolder) return -1
          if (!a.isMainFolder && b.isMainFolder) return 1
          // For folders of the same type, sort alphabetically

...
export default function HierarchyPage() {
        ...
        if (currentUser) {
          ...
          try {
            ...
            if (profile) {
              ...
              console.log('🔧 Hierarchy page - Using fallback role mapping for:', currentUser.email)
              ...
              console.log('✅ Hierarchy page - Fallback role assigned:', originalRole)
            }
          } catch (error) {
            // Database query failed, use fallback mapping
            console.log('❌ Hierarchy page - Database query failed, using fallback for:', currentUser.email)
            const roleMap: { [key: string]: UserRole } = {
              'john@tpnlife.com': 'admin',
              'john+admin@tpnlife.com': 'admin',
              'john+supervisor@tpnlife.com': 'supervisor',
            ...
            originalRole = roleMap[currentUser.email || ''] || 'user'
            console.log('✅ Hierarchy page - Fallback role assigned after error:', originalRole)
          }
          ...
          if (impersonationActive === 'true' && impersonatedUserData) {
            ...
            console.log('👤 Hierarchy page - Using role:', originalRole)
          }
        }
      } catch (error) {
  ...
  if (!user) {
    ...
    router.push('/?redirect=hierarchy')
    return (
      <ThemeProvider>
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
